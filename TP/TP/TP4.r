...
qtheoc <- qnorm(seq(1 / (n + 1), n / (n + 1), 1 / (n + 1)), 0, 1)
plot(qtheoc, sort(x), main = "les quantiles", xlab = "quantiles théoriques", ylab = "quantiles empiriques")
abline(mu, sigma, col = "red")
legend("topleft", legend = c("droite de Henry"), col = c("red"), lty = 1)
abline(v = 1, col = "blue")
abline(h = 5, col = "blue")
